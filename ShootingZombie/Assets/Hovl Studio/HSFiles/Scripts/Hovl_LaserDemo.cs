...
public class Hovl_LaserDemo : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(transform.position, transform.forward, MaxLength, layerMask);
            if (/*Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength)*/hit.collider != null)//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
                ...
                if (!isWitch)
                {
                    ...
                    {
                        hit.collider.GetComponent<TrainManager>().damageGiven = 3;
                        hit.collider.GetComponent<TrainManager>().GetHitBullet();
                    }
                }
                else
                {
                    ...
                    {
                        hit.collider.GetComponent<EnemyBase>().damageGiven = 1;
                        hit.collider.GetComponent<EnemyBase>().TakeDamage(2f);
                    }
                }

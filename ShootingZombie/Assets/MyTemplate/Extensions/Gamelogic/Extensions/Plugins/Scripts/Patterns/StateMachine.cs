...
namespace Gamelogic.Extensions
{
	...
	public class StateMachine<TLabel>
	{
		...
		List<TLabel> _allStateLabels = null;
		public List<TLabel> allStateLabels
        {
			get
            {
				...
				if (_allStateLabels == null || _allStateLabels.Count != stateDictionary.Count)
                {
					_allStateLabels = stateDictionary.Keys.ToList();
				}
				return _allStateLabels;
		...
		public void AddStateInfo(StateInfo<TLabel> stateInfo, bool ovveride = false)
        {
			if (ovveride == false)
            {
				if (stateDictionary.ContainsKey(stateInfo.state))
                {
					return;
                }
            }
			AddState(stateInfo.state, 
				() => stateInfo.onStart?.Invoke(), 
				() => stateInfo.onUpdate?.Invoke(), 
				() => stateInfo.onStop?.Invoke());
		}
		...
		/// a new state.
		/// </summary>
		private void ChangeState(TLabel newStateEnum)
		{
		...
		public bool IsSupportState(TLabel state)
        {
	...
	public class StateInfo<TLabel>
	{
		...
		public StateInfo(TLabel state, UnityEvent onStart = null, UnityEvent onUpdate = null, UnityEvent onStop = null)
		{
			this.state = state;
			this.onStart = onStart;
			this.onUpdate = onUpdate;
